...
class Runner():
  ...
  def run_mapd(self):
    ...
    points, voronoi = mm.call_map_D(self.points,min_number_per_traj,npoints_per_clus,self.sigma_noise,self.delta_t)
    return points, voronoi

  def plot_mapD(self):
    display = self.config['mapD']['display']
    self.figure_mapd = mm.plot_mapD(self.results_mapd,self.voronoi,display)
  ...
  def run(self):
    ...
    if self.config['mapD']['do']:
      ...
      if self.config['mapD']['plot']:
        self.plot_mapD()
    if self.config['data']['path_res_folder'] != None:
      self.save_results()
